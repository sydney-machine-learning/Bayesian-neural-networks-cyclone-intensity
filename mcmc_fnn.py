...
class MCMC:
    def __init__(self, samples, traindata, testdata, topology):
        self.samples = samples  #Number of sample
    ...
    def rmsePerOutput(self, predictions, targets):
		individualRmse = np.zeros(self.topology[2])
		for i in xrange (0, self.topology[2]):
			individualRmse[i] = np.sqrt(((predictions[i] - targets[i])**2).mean())
			
		return individualRmse

    def likelihood_func(self, neuralnet, data, w, tausq):
        y = data[:, self.topology[0]:]
        fx = neuralnet.evaluate_proposal(data, w)
        rmse = self.rmse(fx, y)
        individualrmse = self.rmsePerOutput(fx,y)
        loss = -0.5 * np.log(2 * math.pi * tausq) - 0.5 * np.square(y - fx) / tausq
        return [np.sum(loss), fx, rmse,individualrmse]
    ...
    def sampler(self):
        ...
        rmse_train_individuals = np.zeros((samples,netw[2]))
        rmse_test_individuals = np.zeros((samples,netw[2]))
        ...
        [likelihood, pred_train, rmsetrain,individualrmseTrain] = self.likelihood_func(neuralnet, self.traindata, w, tau_pro)
        [likelihood_ignore, pred_test, rmsetest, individualrmseTest] = self.likelihood_func(neuralnet, self.testdata, w, tau_pro)
        ...
        for i in range(samples - 1):
            ...
            [likelihood_proposal, pred_train, rmsetrain,individualrmseTrain] = self.likelihood_func(neuralnet, self.traindata, w_proposal,
                                                                                tau_pro)
            [likelihood_ignore, pred_test, rmsetest, individualrmseTest] = self.likelihood_func(neuralnet, self.testdata, w_proposal,
                                                                            tau_pro)
            ...
            if u < mh_prob:
                ...
                rmse_train_individuals[i + 1,] = individualrmseTrain
                rmse_test_individuals[i + 1,] = individualrmseTest
                ...
                rmse_train_individuals[i + 1,] = rmse_train_individuals[i,]
                rmse_test_individuals[i + 1,] = rmse_test_individuals[i,]
                
            if (i%50) == 0 :
                print i, rmse_train[i], rmse_test[i]
        ...
        return (pos_w, pos_tau, fxtrain_samples, fxtest_samples, x_train, x_test, rmse_train, rmse_test, accept_ratio, rmse_train_individuals, rmse_test_individuals)


def main():
    outres = open('mcmcresults/resultspriors.txt', 'w')
    outres_train = open('result_train.txt', 'w')
    outres_test = open('result_test.txt', 'w')
    outres_train_std = open('result_train_std.txt', 'w')
    outres_test_std = open('result_test_std.txt', 'w')
    for problem in xrange(3, 4): 
        ...
        [pos_w, pos_tau, fx_train, fx_test, x_train, x_test, rmse_train, rmse_test, accept_ratio,rmse_idv_train, rmse_idv_test] = mcmc.sampler()
        ...
        rmse_tr = mode(rmse_train[int(burnin):])[0]
        rmsetr_std = np.std(rmse_train[int(burnin):])
        rmse_tes = mode(rmse_test[int(burnin):])[0]
        rmsetest_std = np.std(rmse_test[int(burnin):])
        print rmse_tr, rmsetr_std, rmse_tes, rmsetest_std
        
        
        rmse_idv_tr = mode(rmse_idv_train[int(burnin):], axis = 0)[0]
        std_idv_tr = np.std(rmse_idv_train[int(burnin):], axis =0)
        rmse_idv_ts = mode(rmse_idv_test[int(burnin):], axis  = 0)[0]
        std_idv_ts = np.std(rmse_idv_test[int(burnin):], axis = 0)
        
        print '--------------------------------------\n'
        
        #print rmse_idv_train[int(burnin):]
        #print '--------------------------------------\n'
        #print rmse_idv_test[int(burnin):]
        #print '--------------------------------------\n'
        
        #print '--------------------------------------\n'
        print rmse_idv_tr, std_idv_tr
        print rmse_idv_ts , std_idv_ts
       
        #print mode(rmse_idv_train[int(burnin):])
        np.savetxt(outres_train, (rmse_idv_tr), fmt='%1.5f')
        np.savetxt(outres_test, (rmse_idv_ts), fmt='%1.5f')
        np.savetxt(outres_train_std, (std_idv_tr), fmt='%1.5f')
        np.savetxt(outres_test_std, (std_idv_ts), fmt='%1.5f')
